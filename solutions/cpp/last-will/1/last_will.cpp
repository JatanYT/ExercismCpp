...
namespace estate_executor {
    int assemble_account_number(int secret_modifier){
        int bank_number = zhang::bank_number_part(secret_modifier) + khan::bank_number_part(secret_modifier) + garcia::bank_number_part(secret_modifier);
        return bank_number;
    }
    int assemble_code(){
        ...
        int code = red_fragments*blue_fragments;
        return code;
    }
}